 ...
 * 
 */
public class Play implements Variables {
	 ...
	 */
	public static void downloadApp(String doc, int vc, int ot) throws IOException {
		GooglePlayAPI api = createConnection();
		...
		DownloadData data = api.purchaseAndDeliver(doc, vcode, ot);
	...
	/**
	 * Check an app verisoncode and update local db.
	 *
	 * @param doc     packagename
	 * @param localVC local versioncode .
	 */
	public static boolean bstCheckAndUpdateApkInfo(String doc, String localVname, int localVC) {
		boolean bRet = false;
		GooglePlayAPI api = createConnection();
		try {
			DetailsResponse dr = api.details(doc);
			int lastVersionCode = dr.getDocV2().getDetails().getAppDetails().getVersionCode();
			String lastVersionName = dr.getDocV2().getDetails().getAppDetails().getVersionString();
			System.out.println("	get ApkVerInfo=> " + lastVersionCode + " " + lastVersionName);
			if (lastVersionCode > localVC && (!lastVersionName.equals(localVname))) {
				DatabaseManager dbm = GlobalsProvider.getGlobals().get(
						DatabaseManager.class);
				AndroidApp download = new AndroidApp();
				download.setPackageName(doc);
				download.setVersionCode(lastVersionCode);
				download.setName("");
				download.setVersion(lastVersionName);
				dbm.get(AndroidAppDao.class).saveOrUpdate(download);
				dbm.get(PlayAppOwnerDao.class).own(download, getProfile());
				bRet = true;
			}
		} catch (GooglePlayException e) {
			System.err.println(doc + " doesn't exist on Google Play, " +
					"downloadApp has exception =>" + e.getHttpStatus() + "\n" + new String(e.getMessage().getBytes(StandardCharsets.UTF_8)));
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
			//Router.fail("");
	...
	public static void bstInitApp(String doc) {
		DatabaseManager dbm = GlobalsProvider.getGlobals().get(
				DatabaseManager.class);

		AndroidAppDao androidAppDao = dbm.get(AndroidAppDao.class);
		try {
			if (false == androidAppDao.bstIsStored(doc)) {
				System.out.println(doc + " isn't stored in the database, begin download it...");
				//Play.downloadApp(doc,-1,1);
				AndroidApp download = new AndroidApp();
				download.setPackageName(doc);
				download.setVersionCode(-1);
				download.setName("");
				download.setVersion("-1");
				androidAppDao.saveOrUpdate(download);
				dbm.get(PlayAppOwnerDao.class).own(download, getProfile());
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	/**
	 * get downloadUrl for an app.
	 *
	 * @param doc packagename
	 * @param vc  versioncode (maybe -1 to get the latest one).
	 * @param ot  should always be 1.
	 */
	public static DownloadData bstGetDownloadUrl(String doc, int vc, int ot) throws Exception {
	...
	public static String[] bstGetAppVerInfo(String doc) throws Exception {

		String[] appVerInfoArray = null;
		GooglePlayAPI api = createConnection();

		DetailsResponse dr = api.details(doc);
		int lastVersionCode = dr.getDocV2().getDetails().getAppDetails().getVersionCode();
		String lastVersionName = dr.getDocV2().getDetails().getAppDetails().getVersionString();
		appVerInfoArray = new String[2];
		appVerInfoArray[0] = String.valueOf(lastVersionCode);
		appVerInfoArray[1] = lastVersionName;

		return appVerInfoArray;
	}


	/**
	 * Check an app verisoncode.
	 */
	public static boolean bstCheckApkInfo(String localVname, int localVC, String[] remoteAppVerInfoArray) {
		boolean bRet = false;
		try {

			if (remoteAppVerInfoArray != null && remoteAppVerInfoArray.length == 2) {
				int remoteVersionCode = Integer.parseInt(remoteAppVerInfoArray[0]);
				String remoteVersionName = remoteAppVerInfoArray[1];
				if (remoteVersionCode > localVC && (!remoteVersionName.equals(localVname))) {
					bRet = true;
				}
			}
		} catch (Exception e) {
	...
	public static boolean bstCheckProfile() {
		boolean bRet = true;
		PlayProfile pp = getProfile();
		if (pp == null)
			bRet = false;
